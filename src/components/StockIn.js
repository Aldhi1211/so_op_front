...
const StockIn = () => {
    ...
    const getStock = async () => {
        const response = await axiosJWT.get(`http://18.141.194.160/api/stockin?search_query=${keyword}&page=${page}&limit=${limit}`,
            {
                headers: {
    ...
    const displayedData = stockins.slice(page * limit, (page + 1) * limit);
        ...
        <div>
            ...
            <div className="container mt-5">
                ...
                <div className="is-flex is-justify-content-space-between is-align-items-center mb-4">
                    <h2 className="title is-4 has-text-black">Stock In List</h2>
                </div>
                    ...
                    <table className="table is-fullwidth is-bordered is-hoverable custom-hover-table">
                        <thead>
                            <tr className="has-background-grey">
                                ...
                                <th onClick={() => handleSort('tanggal_beli')}>
                                    <div className="is-flex is-justify-content-space-between is-align-items-center">
                                        Tanggal Beli
                                        {sortConfig.key === 'tanggal_beli' && (sortConfig.direction === 'asc' ? (
                                            <i className="fa fa-arrow-up"></i>
                                        ) : (
                                            <i className="fa fa-arrow-down"></i>
                                        ))}
                        ...
                        <tbody className="has-background-light">
                            {Array.isArray(stockins) && stockins.map((stockin, index) => (
                                <tr key={stockin.id}>
                                    <td className="has-text-black">{index + 1}</td>
                                    <td className="has-text-black">{stockin.barang?.name || 'Tidak Diketahui'}</td>
                                    <td className="has-text-black">
                                        {new Intl.DateTimeFormat('id-ID', {
                                            year: 'numeric',
                                            month: 'long',
                                            day: 'numeric',
                                        }).format(new Date(stockin.tanggal_beli))}
                                    </td>

                                    <td className="has-text-black">{stockin.quantity}</td>
                                    <td className="has-text-black">{stockin.satuan}</td>
                                    <td className="has-text-black">{stockin.submitted_by}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
